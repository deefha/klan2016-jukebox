...
DisplayAccessor = function(doGetSongIndex) {
	this.doGetSongIndex = doGetSongIndex;
};
...
// 	this.WIDTH = document.getElementById('canvas-spectrum').offsetWidth;
// 	this.HEIGHT = document.getElementById('canvas-spectrum').offsetHeight;
	this.WIDTH = 300;
	this.HEIGHT = 95;
	...
	// rendering should take no longer than 10% of the available time (e.g. 3-4 millis per frame)
	this.timeLimit = 1000 / 60 * cpuLimit;

	this.canvasSpectrum = document.getElementById('canvas-spectrum');
...
SongDisplay.prototype = {
	...
	redrawSpectrum: function() {
		// with low enough load the browser should be able to render 60fps:
		// the load from the music generation cannot be changed but the load from the rendering can
...
// 			this.ctxSpectrum.lineCap = 'round';
			...
			if (this.colors) {
				...
				if ((this.barType & 0x1) == 1) {
					...
					for (var i = 0; i < numBars; ++i) {
						...
						// computationally much less expensive than previous impl with use of a scaled image
						this.ctxSpectrum.fillStyle = this.colorGradient(this.colors, p / numBars);
					...
					for (var i = 0; i < numBars; ++i) {
						...
						this.ctxSpectrum.fillStyle= '#ffffff';
				...
				if (this.backgroundImg) {
					if (this.barType == 1) {
						...
						for (var i = 0; i < numBars; ++i) {
							...
							this.ctxSpectrum.drawImage(this.backgroundImg, 0, 0, this.barWidth, 255, i * this.barSpacing, o, this.barWidth, 30 * (scale * 1.4));
						}
					}
					else {
						...
						for (var i = 0; i < numBars; ++i) {
							var magnitude = freqByteData[i + OFFSET] * this.HEIGHT / 255;
							o = Math.round(this.HEIGHT - magnitude);
							this.ctxSpectrum.drawImage(this.backgroundImg, i * sliceW, 0, sliceW, imgH, i * this.barSpacing, o, this.barWidth, Math.round(magnitude));
						}
					}
				}
			}
	...
	text: function(ctx, t, x, y) {
		...
		t = t.replace(/&#.*?;/g, function(needle) {
			// replace html chars "&#111;"
	...
	redrawSongInfo: function() {
		...
		var songIndex = this.displayAccessor.getSongIndex();

		$('#playlist li a').each(function() {
			$(this).removeClass('active');
			if ($(this).data('index') == songIndex + 1) {
